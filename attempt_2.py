...
def apply_gaussian_blur(image, sigma=0.5, kernel_size=3):
...
def apply_cropping(image, crop_percentage, mode='replicate', seed=None):
    ...
    pad_right = w - end_x
    if mode == 'zero':
        padded = cv2.copyMakeBorder(cropped, pad_top, pad_bottom, pad_left, pad_right, borderType=cv2.BORDER_CONSTANT,
                                    value=0)
    elif mode == 'replicate':
...
def objective_function(alpha, host_band, wm_norm, host, coeffs_copy):
    try:
        ...
        coeffs_temp = [np.copy(c) if isinstance(c, np.ndarray) else tuple(np.copy(sub) for sub in c) for c in
                       coeffs_copy]
...
# host_path = "images/woman.jpg"
# host_path = "images/texture.tiff"
# host_path = "images/medical.jpg"
# host_path = "images/woman.tiff"
# host_path = "images/pepper.tiff"
# host_path = "images/house.tiff"

wm_path = "watermark.jpg"
output_folder = "output_optimized_balanced"
...
try:
    ...
    wm_band, orig_S, wm_U, wm_S, wm_Vt, host_U, host_Vt = embed_watermark_band(host_band, wm_norm_scrambled,
                                                                               optimal_alpha)
    ...
    watermarked_uint8 = apply_gaussian_noise(watermarked_uint8, noise_std=0.1, seed=42)
    ...
    """
    # --- Apply Poisson noise ---
    watermarked_uint8 = apply_poisson_noise(watermarked_uint8, scale_factor=1.0, seed=42)
    ...
    watermarked_uint8 = apply_salt_and_pepper_noise(watermarked_uint8, prob=0.001, seed=42)
    ...
    """
    # --- Apply speckle noise ---
    watermarked_uint8 = apply_speckle_noise(watermarked_uint8, variance=0.001, seed=42)
    ...
    watermarked_uint8 = apply_cropping(watermarked_uint8, crop_percentage=0.03, seed=42)
    ...
    watermarked_uint8 = apply_scaling(watermarked_uint8, scale_factor=0.5)
    ...
    watermarked_uint8 = apply_jpeg_compression(watermarked_uint8, quality=70)
    ...
    watermarked_uint8 = apply_gaussian_blur(watermarked_uint8, sigma=0.5, kernel_size=3)
    ...
    watermarked_uint8 = apply_hybrid_attack_1(watermarked_uint8, noise_std=0.1, jpeg_quality=70, seed=42)
    ...
    watermarked_uint8 = apply_hybrid_attack_2(watermarked_uint8, prob=0.001, angle=0.01, seed=42)
    ...
    watermarked_uint8 = apply_hybrid_attack_3(watermarked_uint8, prob=0.001, sigma=0.5, kernel_size=3, seed=42)
    ...
    watermarked_uint8 = apply_hybrid_attack_4(watermarked_uint8, crop_percentage=0.03, quality=70, seed=42)
    ...
    watermarked_uint8 = apply_hybrid_attack_5(watermarked_uint8, variance=0.001, prob=0.001, seed=42)
    ...
    coeffs_copy_recovered = [np.copy(c) if isinstance(c, np.ndarray) else tuple(np.copy(sub) for sub in c) for c in
                             recoeffs]
    ...
    # Save results
    cv2.imwrite(os.path.join(output_folder, "LL3_watermarked.png"), watermarked_uint8)
    cv2.imwrite(os.path.join(output_folder, "LL3_reconstructed.png"), recovered_host_uint8)
    cv2.imwrite(os.path.join(output_folder, "LL3_extracted_watermark.png"), denormalize_image(wm_extracted_unscrambled))

    end_time = time.time()
    print(f"\n--- Execution Results for LL3 Band ---")
